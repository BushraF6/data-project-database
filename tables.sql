...
SELECT DISTINCT country,count(country) AS no_of_umpires FROM umpires WHERE country!=' India' GROUP BY country;
/* 4.Number of matches played by team by season. */
SELECT season,team1 as team, count(team1)+count(team2) as no_of_matches FROM matches GROUP BY team,season ORDER BY season;
